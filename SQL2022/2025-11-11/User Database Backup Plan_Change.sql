...
DECLARE @jobId BINARY(16);
	...
	BEGIN
		...
		BEGIN TRY
			...
			BACKUP DATABASE @DB_Name TO DISK = @Disk_Full
				WITH  RETAINDAYS = 21, NOFORMAT, NOINIT,  COMPRESSION,ENCRYPTION (ALGORITHM = AES_256, SERVER CERTIFICATE = AIDBA_Encryption), 
	...
	BEGIN TRY
		...
		BEGIN
			...
			SET @stmt = ''
                BACKUP DATABASE [''+@DB_Name+''] TO DISK = ''''''+@Disk_Full+''''''
                WITH RETAINDAYS = 21, FORMAT, INIT,  COMPRESSION, ENCRYPTION (ALGORITHM = AES_256, SERVER CERTIFICATE = AIDBA_Encryption), 
...
--Backup Conditions:
 ...
 --Shrink Condition
	...
	USE [msdb]
	IF EXISTS (SELECT * FROM sys.databases WHERE name = 'ReportDB' and recovery_model = 1)
	BEGIN
	BEGIN TRANSACTION
	/****** Object:  Job [AIDBA ReportDB Full Backup]    Script Date: 03/01/2019 11:03:37 AM ******/
	SELECT @ReturnCode = 0
	/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 03/01/2019 11:03:37 AM ******/
	IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'[Uncategorized (Local)]' AND category_class=1)
	BEGIN
	...
	SELECT @jobId = NULL
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'AIDBA ReportDB Full Backup', 
			@enabled=1, 
			@notify_level_eventlog=0, 
			@notify_level_email=0, 
			@notify_level_netsend=0, 
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Database Integrity & Full Backup', 
			...
			@os_run_priority=0, @subsystem=N'TSQL', 
			@command=N'DECLARE @Disk_Full nvarchar(max);
	DECLARE @DB_Name sysname=''ReportDB'';
	DECLARE @stmt nvarchar(max);
	SELECT @Disk_Full = N''D:\AIDBA_Backups\Full\''+@DB_Name+''_FULL_''+Convert(Nvarchar,SysDateTime(),112)+''.bak'';
	...
	/****** Object:  Job [AIDBA ReportDB Log Backup]    Script Date: 03/01/2019 11:06:51 AM ******/
	...
	SELECT @jobId = NULL
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'AIDBA ReportDB Log Backup', 
			@enabled=1, 
			@notify_level_eventlog=0, 
			@notify_level_email=0, 
			@notify_level_netsend=0, 
...
--Backup Conditions:
 ...
 --Shrink Condition
	...
	USE [msdb]
	IF EXISTS (SELECT * FROM sys.databases WHERE name = 'reportdb3' and recovery_model = 1)
	BEGIN
	BEGIN TRANSACTION
	/****** Object:  Job [AIDBA reportdb3 Full Backup]    Script Date: 03/01/2019 11:03:37 AM ******/
	SELECT @ReturnCode = 0
	/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 03/01/2019 11:03:37 AM ******/
	IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'[Uncategorized (Local)]' AND category_class=1)
	BEGIN
	...
	SELECT @jobId = NULL
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'AIDBA reportdb3 Full Backup', 
			@enabled=1, 
			@notify_level_eventlog=0, 
			@notify_level_email=0, 
			@notify_level_netsend=0, 
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Database Integrity & Full Backup', 
			...
			@os_run_priority=0, @subsystem=N'TSQL', 
			@command=N'DECLARE @Disk_Full nvarchar(max);
	DECLARE @DB_Name sysname=''reportdb3'';
	DECLARE @stmt nvarchar(max);
	SELECT @Disk_Full = N''D:\AIDBA_Backups\Full\''+@DB_Name+''_FULL_''+Convert(Nvarchar,SysDateTime(),112)+''.bak'';
	...
	/****** Object:  Job [AIDBA reportdb3 Log Backup]    Script Date: 03/01/2019 11:06:51 AM ******/
	...
	SELECT @jobId = NULL
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'AIDBA reportdb3 Log Backup', 
			@enabled=1, 
			@notify_level_eventlog=0, 
			@notify_level_email=0, 
			@notify_level_netsend=0, 
...
--Backup Conditions:
 ...
 --Shrink Condition
	...
	BEGIN
	EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'[Uncategorized (Local)]'
	IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback9
