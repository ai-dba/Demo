...
AND dp.name = '##MS_PolicyEventProcessingLogin##'
	...
	BEGIN
		...
		BEGIN
			...
			PRINT 'USER [##MS_PolicyEventProcessingLogin##] ON DATABASE [master] IS MAPPED TO CORRESPONDING LOGIN.';
		END
		ELSE
		BEGIN
			USE [master];
			DROP USER [##MS_PolicyEventProcessingLogin##];
			PRINT N'USER [##MS_PolicyEventProcessingLogin##] ON DATABASE [master] IS DROPED. ANOTHER USER IS MAPPED TO THE CORRESPONDING LOGIN.';
		END
	END
UserNotOrphaned1:
	PRINT N'USER [##MS_PolicyEventProcessingLogin##] ON DATABASE [master] IS NOT ORPHANED.';
END TRY
BEGIN CATCH
	PRINT N'USER [##MS_PolicyEventProcessingLogin##] ON DATABASE [master] IS ORPHANED AND CANNOT BE DROPED OR MAPPED.';
END CATCH;
