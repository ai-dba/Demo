    ...
    (
        SELECT DISTINCT ST2.Index_Name, 
            (
				...
				WHERE I.type_desc = ''NONCLUSTERED'' AND F.name=''AIDBA_NCIX'' AND IC.is_included_column = 0
...
FROM
	(
		...
		(
			...
			INNER JOIN sys.columns AS C ON IC.column_id=C.column_id AND C.object_id=I.object_id
			INNER JOIN sys.filegroups AS F ON F.data_space_id=I.data_space_id
			WHERE I.type_desc = ''NONCLUSTERED'' AND F.name=''AIDBA_NCIX'' AND IC.is_included_column = 1
				AND I.is_primary_key = 0 AND I.is_unique_constraint = 0 AND I.is_unique = 0) ST1
			WHERE ST1.Index_Name = ST2.Index_Name
			ORDER BY ST1.Index_Name
			FOR XML PATH ('''')
...
DECLARE CUR1_[*] CURSOR FOR	
		...
		WHERE I.type_desc = ''NONCLUSTERED'' AND F.name=''AIDBA_NCIX'' AND I.is_primary_key = 0 
		...
		WHILE (@@FETCH_STATUS = 0)
			...
			SET @Script=ISNULL(''
					CREATE NONCLUSTERED INDEX [''+@Index_Name_[*]+''] ON [''+@Schema_Name_[*]+''].[''+@Table_Name_[*]+''] (''+@Key_Columns_[*]+'') WITH (DROP_EXISTING = ON);
			'','''')
			END
			ELSE
			BEGIN
			SET @Script=ISNULL(''
					CREATE NONCLUSTERED INDEX [''+@Index_Name_[*]+''] ON [''+@Schema_Name_[*]+''].[''+@Table_Name_[*]+''] (''+@Key_Columns_[*]+'') INCLUDE (''+@Included_Columns_[*]+'') WITH (DROP_EXISTING = ON);
			'','''')
			END
