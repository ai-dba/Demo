...
USE [master];
	...
	WITH PRIVATE KEY (FILE = 'C:\AIDBA_SQL_Certificate_Backups\AIDBA_PKey' , ENCRYPTION BY PASSWORD = 'Pa$$w0rd')
				...
				BEGIN TRY
					...
					IF (SELECT is_encrypted FROM sys.databases WHERE NAME = 'AdventureWorks_20251212132121' AND state = 0 ) = 0
					BEGIN
					CREATE DATABASE ENCRYPTION KEY  WITH ALGORITHM = AES_256  ENCRYPTION BY SERVER CERTIFICATE AIDBA_Encryption;
					ALTER DATABASE [AdventureWorks_20251212132121]  SET ENCRYPTION ON; 
					PRINT 'Database [AdventureWorks_20251212132121] has been encrypted.'
					END
					ELSE
						PRINT 'Database AdventureWorks_20251212132121 is already encrypted or is not online.'
				END TRY
				BEGIN CATCH
					PRINT 'Database [AdventureWorks_20251212132121] could not be encrypted.'
				END CATCH;
				
				BEGIN TRY
					USE [???];
					IF (SELECT is_encrypted FROM sys.databases WHERE NAME = '???' AND state = 0 ) = 0
					BEGIN
					CREATE DATABASE ENCRYPTION KEY  WITH ALGORITHM = AES_256  ENCRYPTION BY SERVER CERTIFICATE AIDBA_Encryption;
					ALTER DATABASE [???]  SET ENCRYPTION ON; 
					PRINT 'Database [???] has been encrypted.'
					END
					ELSE
						PRINT 'Database ??? is already encrypted or is not online.'
				END TRY
				BEGIN CATCH
					PRINT 'Database [???] could not be encrypted.'
				END CATCH;
