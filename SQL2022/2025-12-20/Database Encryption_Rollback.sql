	...
	BEGIN TRY
		USE [master];
		IF (SELECT is_encrypted FROM sys.databases WHERE NAME = '???' AND state = 0 ) = 1
		BEGIN
			...
			BEGIN
				WAITFOR DELAY '00:15';
				IF (SELECT encryption_state FROM sys.dm_database_encryption_keys WHERE database_id = DB_ID('???')) = 1 
					BREAK;
				@i += 1;
			END;
			USE [???];
		...
		ELSE
		BEGIN
			PRINT 'Database ??? is already decrypted or is not online.'
		END
	END TRY
	BEGIN CATCH
		PRINT 'Database [???] could not be decrypted. '+ERROR_MESSAGE();
	END CATCH;
