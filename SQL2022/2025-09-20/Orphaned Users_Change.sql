...
AND dp.name = 'AIDBA'
	...
	SELECT @Lsid = NULL
	SELECT @Lsid = sid FROM sys.server_principals WHERE name = 'AIDBA'
	IF @Lsid IS NULL
	BEGIN
		CREATE LOGIN [AIDBA] WITH PASSWORD = 'Pa$$w0rd', SID = 0xC1C31E62E07E3A46BCCA533AA68E8D5E;
		PRINT N'LOGIN [AIDBA] IS CREATED AND MAPPED TO CORRESPONDING USER.';
	END
	ELSE 
	BEGIN
		IF NOT EXISTS
		...
		BEGIN
			...
			PRINT 'USER [AIDBA] ON DATABASE [master] IS MAPPED TO CORRESPONDING LOGIN.';
		END
		ELSE
		BEGIN
			USE [master];
			DROP USER [AIDBA];
			PRINT N'USER [AIDBA] ON DATABASE [master] IS DROPED. ANOTHER USER IS MAPPED TO THE CORRESPONDING LOGIN.';
		END
	END
UserNotOrphaned1:
	PRINT N'USER [AIDBA] ON DATABASE [master] IS NOT ORPHANED.';
END TRY
BEGIN CATCH
	PRINT N'USER [AIDBA] ON DATABASE [master] IS ORPHANED AND CANNOT BE DROPED OR MAPPED.';
END CATCH;
