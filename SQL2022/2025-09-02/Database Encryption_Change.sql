...
USE [master];
	...
	WITH PRIVATE KEY (FILE = 'C:\AIDBA_SQL_Certificate_Backups\AIDBA_PKey' , ENCRYPTION BY PASSWORD = 'Pa$$w0rd')
				...
				BEGIN TRY
					...
					IF (SELECT is_encrypted FROM sys.databases WHERE NAME = 'NewAdventureWorks' AND state = 0 ) = 0
					BEGIN
					CREATE DATABASE ENCRYPTION KEY  WITH ALGORITHM = AES_256  ENCRYPTION BY SERVER CERTIFICATE AIDBA_Encryption;
					ALTER DATABASE [NewAdventureWorks]  SET ENCRYPTION ON; 
					PRINT 'Database [NewAdventureWorks] has been encrypted.'
					END
					ELSE
						PRINT 'Database NewAdventureWorks is already encrypted or is not online.'
				END TRY
				BEGIN CATCH
					PRINT 'Database [NewAdventureWorks] could not be encrypted.'
				END CATCH;
				
				BEGIN TRY
					USE [AZUREADVWORKS2017];
					IF (SELECT is_encrypted FROM sys.databases WHERE NAME = 'AZUREADVWORKS2017' AND state = 0 ) = 0
					BEGIN
					CREATE DATABASE ENCRYPTION KEY  WITH ALGORITHM = AES_256  ENCRYPTION BY SERVER CERTIFICATE AIDBA_Encryption;
					ALTER DATABASE [AZUREADVWORKS2017]  SET ENCRYPTION ON; 
					PRINT 'Database [AZUREADVWORKS2017] has been encrypted.'
					END
					ELSE
						PRINT 'Database AZUREADVWORKS2017 is already encrypted or is not online.'
				END TRY
				BEGIN CATCH
					PRINT 'Database [AZUREADVWORKS2017] could not be encrypted.'
				END CATCH;
				...
				BEGIN TRY
					USE [test55666];
					IF (SELECT is_encrypted FROM sys.databases WHERE NAME = 'test55666' AND state = 0 ) = 0
					BEGIN
					CREATE DATABASE ENCRYPTION KEY  WITH ALGORITHM = AES_256  ENCRYPTION BY SERVER CERTIFICATE AIDBA_Encryption;
					ALTER DATABASE [test55666]  SET ENCRYPTION ON; 
					PRINT 'Database [test55666] has been encrypted.'
					END
					ELSE
						PRINT 'Database test55666 is already encrypted or is not online.'
				END TRY
				BEGIN CATCH
					PRINT 'Database [test55666] could not be encrypted.'
				END CATCH;
				...
				BEGIN TRY
					USE [AdvNewDB2];
					IF (SELECT is_encrypted FROM sys.databases WHERE NAME = 'AdvNewDB2' AND state = 0 ) = 0
					BEGIN
					CREATE DATABASE ENCRYPTION KEY  WITH ALGORITHM = AES_256  ENCRYPTION BY SERVER CERTIFICATE AIDBA_Encryption;
					ALTER DATABASE [AdvNewDB2]  SET ENCRYPTION ON; 
					PRINT 'Database [AdvNewDB2] has been encrypted.'
					END
					ELSE
						PRINT 'Database AdvNewDB2 is already encrypted or is not online.'
				END TRY
				BEGIN CATCH
					PRINT 'Database [AdvNewDB2] could not be encrypted.'
				END CATCH;
				...
				BEGIN TRY
					USE [AdvDest20240317];
					IF (SELECT is_encrypted FROM sys.databases WHERE NAME = 'AdvDest20240317' AND state = 0 ) = 0
					BEGIN
					CREATE DATABASE ENCRYPTION KEY  WITH ALGORITHM = AES_256  ENCRYPTION BY SERVER CERTIFICATE AIDBA_Encryption;
					ALTER DATABASE [AdvDest20240317]  SET ENCRYPTION ON; 
					PRINT 'Database [AdvDest20240317] has been encrypted.'
					END
					ELSE
						PRINT 'Database AdvDest20240317 is already encrypted or is not online.'
				END TRY
				BEGIN CATCH
					PRINT 'Database [AdvDest20240317] could not be encrypted.'
				END CATCH;
				...
				BEGIN TRY
					USE [DemoADV];
					IF (SELECT is_encrypted FROM sys.databases WHERE NAME = 'DemoADV' AND state = 0 ) = 0
					BEGIN
					CREATE DATABASE ENCRYPTION KEY  WITH ALGORITHM = AES_256  ENCRYPTION BY SERVER CERTIFICATE AIDBA_Encryption;
					ALTER DATABASE [DemoADV]  SET ENCRYPTION ON; 
					PRINT 'Database [DemoADV] has been encrypted.'
					END
					ELSE
						PRINT 'Database DemoADV is already encrypted or is not online.'
				END TRY
				BEGIN CATCH
					PRINT 'Database [DemoADV] could not be encrypted.'
				END CATCH;
