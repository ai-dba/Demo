	...
	BEGIN TRY
		USE [master];
		IF (SELECT is_encrypted FROM sys.databases WHERE NAME = 'NewAdventureWorks' AND state = 0 ) = 1
		BEGIN
			ALTER DATABASE [NewAdventureWorks]  SET ENCRYPTION OFF; 
			DECLARE @i int  = 0;
			WHILE @i < 5
			BEGIN
				WAITFOR DELAY '00:15';
				IF (SELECT encryption_state FROM sys.dm_database_encryption_keys WHERE database_id = DB_ID('NewAdventureWorks')) = 1 
					BREAK;
				@i += 1;
			END;
			USE [NewAdventureWorks];
			DROP DATABASE ENCRYPTION KEY;
			PRINT 'Database [NewAdventureWorks] has been decrypted.'
		END
		ELSE
		BEGIN
			PRINT 'Database NewAdventureWorks is already decrypted or is not online.'
		END
	END TRY
	BEGIN CATCH
		PRINT 'Database [NewAdventureWorks] could not be decrypted. '+ERROR_MESSAGE();
	END CATCH;
	
	BEGIN TRY
		USE [master];
		IF (SELECT is_encrypted FROM sys.databases WHERE NAME = 'AZUREADVWORKS2017' AND state = 0 ) = 1
		BEGIN
			ALTER DATABASE [AZUREADVWORKS2017]  SET ENCRYPTION OFF; 
			DECLARE @i int  = 0;
			WHILE @i < 5
			BEGIN
				WAITFOR DELAY '00:15';
				IF (SELECT encryption_state FROM sys.dm_database_encryption_keys WHERE database_id = DB_ID('AZUREADVWORKS2017')) = 1 
					BREAK;
				@i += 1;
			END;
			USE [AZUREADVWORKS2017];
			DROP DATABASE ENCRYPTION KEY;
			PRINT 'Database [AZUREADVWORKS2017] has been decrypted.'
		END
		ELSE
		BEGIN
			PRINT 'Database AZUREADVWORKS2017 is already decrypted or is not online.'
		END
	END TRY
	BEGIN CATCH
		PRINT 'Database [AZUREADVWORKS2017] could not be decrypted. '+ERROR_MESSAGE();
	END CATCH;
	
	BEGIN TRY
		USE [master];
		IF (SELECT is_encrypted FROM sys.databases WHERE NAME = 'Adv2022ShalevSoft' AND state = 0 ) = 1
		BEGIN
			ALTER DATABASE [Adv2022ShalevSoft]  SET ENCRYPTION OFF; 
			DECLARE @i int  = 0;
			WHILE @i < 5
			BEGIN
				WAITFOR DELAY '00:15';
				IF (SELECT encryption_state FROM sys.dm_database_encryption_keys WHERE database_id = DB_ID('Adv2022ShalevSoft')) = 1 
					BREAK;
				@i += 1;
			END;
			USE [Adv2022ShalevSoft];
			DROP DATABASE ENCRYPTION KEY;
			PRINT 'Database [Adv2022ShalevSoft] has been decrypted.'
		END
		ELSE
		BEGIN
			PRINT 'Database Adv2022ShalevSoft is already decrypted or is not online.'
		END
	END TRY
	BEGIN CATCH
		PRINT 'Database [Adv2022ShalevSoft] could not be decrypted. '+ERROR_MESSAGE();
	END CATCH;
	
	BEGIN TRY
		USE [master];
		IF (SELECT is_encrypted FROM sys.databases WHERE NAME = 'test55666' AND state = 0 ) = 1
		BEGIN
			ALTER DATABASE [test55666]  SET ENCRYPTION OFF; 
			DECLARE @i int  = 0;
			WHILE @i < 5
			BEGIN
				WAITFOR DELAY '00:15';
				IF (SELECT encryption_state FROM sys.dm_database_encryption_keys WHERE database_id = DB_ID('test55666')) = 1 
					BREAK;
				@i += 1;
			END;
			USE [test55666];
			DROP DATABASE ENCRYPTION KEY;
			PRINT 'Database [test55666] has been decrypted.'
		END
		ELSE
		BEGIN
			PRINT 'Database test55666 is already decrypted or is not online.'
		END
	END TRY
	BEGIN CATCH
		PRINT 'Database [test55666] could not be decrypted. '+ERROR_MESSAGE();
	END CATCH;
	...
	BEGIN TRY
		USE [master];
		IF (SELECT is_encrypted FROM sys.databases WHERE NAME = 'AdvNewDB2' AND state = 0 ) = 1
		BEGIN
			ALTER DATABASE [AdvNewDB2]  SET ENCRYPTION OFF; 
			DECLARE @i int  = 0;
			WHILE @i < 5
			BEGIN
				WAITFOR DELAY '00:15';
				IF (SELECT encryption_state FROM sys.dm_database_encryption_keys WHERE database_id = DB_ID('AdvNewDB2')) = 1 
					BREAK;
				@i += 1;
			END;
			USE [AdvNewDB2];
			DROP DATABASE ENCRYPTION KEY;
			PRINT 'Database [AdvNewDB2] has been decrypted.'
		END
		ELSE
		BEGIN
			PRINT 'Database AdvNewDB2 is already decrypted or is not online.'
		END
	END TRY
	BEGIN CATCH
		PRINT 'Database [AdvNewDB2] could not be decrypted. '+ERROR_MESSAGE();
	END CATCH;
	...
	BEGIN TRY
		USE [master];
		IF (SELECT is_encrypted FROM sys.databases WHERE NAME = 'AdvDest20240317' AND state = 0 ) = 1
		BEGIN
			ALTER DATABASE [AdvDest20240317]  SET ENCRYPTION OFF; 
			DECLARE @i int  = 0;
			WHILE @i < 5
			BEGIN
				WAITFOR DELAY '00:15';
				IF (SELECT encryption_state FROM sys.dm_database_encryption_keys WHERE database_id = DB_ID('AdvDest20240317')) = 1 
					BREAK;
				@i += 1;
			END;
			USE [AdvDest20240317];
			DROP DATABASE ENCRYPTION KEY;
			PRINT 'Database [AdvDest20240317] has been decrypted.'
		END
		ELSE
		BEGIN
			PRINT 'Database AdvDest20240317 is already decrypted or is not online.'
		END
	END TRY
	BEGIN CATCH
		PRINT 'Database [AdvDest20240317] could not be decrypted. '+ERROR_MESSAGE();
	END CATCH;
	...
	BEGIN TRY
		USE [master];
		IF (SELECT is_encrypted FROM sys.databases WHERE NAME = 'DemoADV' AND state = 0 ) = 1
		BEGIN
			ALTER DATABASE [DemoADV]  SET ENCRYPTION OFF; 
			DECLARE @i int  = 0;
			WHILE @i < 5
			BEGIN
				WAITFOR DELAY '00:15';
				IF (SELECT encryption_state FROM sys.dm_database_encryption_keys WHERE database_id = DB_ID('DemoADV')) = 1 
					BREAK;
				@i += 1;
			END;
			USE [DemoADV];
			DROP DATABASE ENCRYPTION KEY;
			PRINT 'Database [DemoADV] has been decrypted.'
		END
		ELSE
		BEGIN
			PRINT 'Database DemoADV is already decrypted or is not online.'
		END
	END TRY
	BEGIN CATCH
		PRINT 'Database [DemoADV] could not be decrypted. '+ERROR_MESSAGE();
	END CATCH;
