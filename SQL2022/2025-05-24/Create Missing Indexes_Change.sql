...
/*
    ...
    Recommendation Time: May 24 2025  1:02AM
...
BEGIN TRY
BEGIN TRAN
	IF NOT EXISTS ( SELECT * FROM sys.indexes WHERE NAME = N'IX_AIDBA_1_20250524')
	BEGIN
		CREATE NONCLUSTERED INDEX IX_AIDBA_1_20250524
		...
		WHERE i.name <> N'IX_AIDBA_1_20250524' AND i.object_id = OBJECT_ID('[AdventureWorks].[Sales].[SalesOrderHeader]') 
		AND EXISTS
		(SELECT *
		FROM sys.indexes i2
		INNER JOIN sys.index_columns ic2
		ON i2.index_id = ic2.index_id AND i2.object_id = ic2.object_id 
		WHERE i2.name = N'IX_AIDBA_1_20250524' AND i2.object_id = OBJECT_ID('[AdventureWorks].[Sales].[SalesOrderHeader]')
		...
		IF @iid IS NOT NULL
		BEGIN
			DROP INDEX IX_AIDBA_1_20250524 ON [AdventureWorks].[Sales].[SalesOrderHeader];
			PRINT N'HYPOTHETICAL INDEX IX_AIDBA_1_20250524 ON [AdventureWorks].[Sales].[SalesOrderHeader] IS DROPPED DUE TO DUPLICATION.';
			IF @is_disable = 1
			BEGIN
		...
		BEGIN
			...
			PRINT N'INDEX IX_AIDBA_1_20250524 ON [AdventureWorks].[Sales].[SalesOrderHeader] IS CREATED.';
		END
	END
	ELSE
	BEGIN
		PRINT N'There is already an index named IX_AIDBA_1_20250524 in the database.';
	END
COMMIT TRAN
END TRY
BEGIN CATCH
IF @@TRANCOUNT > 0
ROLLBACK TRAN
	PRINT N'INDEX IX_AIDBA_1_20250524 ON [AdventureWorks].[Sales].[SalesOrderHeader] CANNOT BE CREATED.';
END CATCH;
